...
const IncomePage = () => {
  ...
  const categories = ['Salary', 'Freelance', 'Investments', 'Gifts', 'Refunds', 'Other'];

  // Fetch incomes and accounts
  const fetchPageData = async () => {
    ...
    try {
      ...
      toast.error('Failed to load income data and accounts');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEditClick = (income) => {
    ...
    setValue('accountId', income.accountId?._id || income.accountId);
    setValue('date', income.date.substring(0, 10));
    setModalOpen(true);
  };
  ...
  const onSubmit = async (data) => {
    try {
      if (editingIncome) {
        // Edit flow
        const res = await api.put(`/income/${editingIncome._id}`, data);
        if (res.data.success) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-800 dark:text-white flex items-center">
            <TrendingUp className="w-7 h-7 mr-2.5 text-emerald-500" />
            Income Management
          </h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Track and log your salary, dividends, and other earnings
          </p>
        </div>
        <button
      ...
      <div className="bg-white dark:bg-[#161e2f] border border-slate-100 dark:border-slate-800/80 rounded-2xl card-shadow overflow-hidden">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 space-y-3">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">Loading incoming flows...</p>
          </div>
                  ...
                  >
                    ...
                    <td className="px-6 py-4.5 text-slate-600 dark:text-slate-350">
                      <span className="flex items-center text-xs font-medium">
                        ...
                        {income.accountId?.name || 'Main Checking'}
                      </span>
                    </td>
                    <td className="px-6 py-4.5">
                      <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-emerald-50 text-emerald-700 dark:bg-emerald-950/20 dark:text-emerald-400">
                        {income.category}
                      </span>
                    </td>
          ...
          <div className="text-center py-16 text-slate-400 dark:text-slate-500">
            No income entries found matching your search.
          </div>
        )}
      </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="mt-5 space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  {...register('accountId', { required: 'Please select a deposit account' })}
                >
